    ...
    if (typeof op.insert === "object" && op.insert !== null) {
      ...
      if ("attributes" in op) {
        ...
        if ("link" in op.attributes) {
          op.insert = `<a href="${op.attributes.link}">${op.insert}</a>`;
        }
        ret += op.insert;
      } else {
...
exports.convertDeltaToHtmlUseHref = (delta) => {
  let ret = "";
  if ("ops" in delta) delta = delta.ops;
  for (let i = 0; i < delta.length; i++) {
    const op = delta[i];
    if (typeof op.insert === "object" && op.insert !== null) {
      if ("fileBlot" in op.insert) {
        ret += `<a href="${op.insert.fileBlot.href}">${
          op.insert.fileBlot.fileName
        }</a>`;
      } else if ("mention" in op.insert) {
        ret += `@${op.insert.mention.value}`;
      } else if ("image" in op.insert) {
        ret += `<img src="${op.insert.image}">`;
      } else if ("video" in op.insert) {
        ret += `<a href="${op.insert.video}">视频</a>`;
      }
    } else if (typeof op.insert === "string") {
      op.insert = escape(op.insert);
      if ("attributes" in op) {
